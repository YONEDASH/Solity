...
import de.yonedash.solity.graphics.TextureIndividual;
...
public class EntityProjectile extends EntityBase {
    ...
    public EntityProjectile(Texture texture, BoundingBox boundingBox, Entity shooter, double rotation, double moveSpeed, double damage, double maxDistance, boolean spin) {
        super(boundingBox);
        this.texture = new TextureIndividual(texture);
        this.origin = boundingBox.center();
        this.shooter = shooter;
    ...
    public void draw(Scene scene, Graphics2D g2d, double dt) {
        ...
        if (spin) GraphicsUtils.rotate(g2d, d * 360.0 * 3, scene.scaleToDisplay(center.x), scene.scaleToDisplay(center.y));
        g2d.drawImage(
    ...
    public boolean collide(Scene scene, Entity entity) {
        ...
        if (entity instanceof EntityPlayer)
            damageMultiplier = scene.instance.world.saveGame.getDifficulty();
        ...
        entityDamageable.setHealth(entityDamageable.getHealth() - (damage * damageMultiplier));
